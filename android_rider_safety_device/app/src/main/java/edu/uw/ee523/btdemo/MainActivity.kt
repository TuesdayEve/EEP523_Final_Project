...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var alterMessage: TextView
    private var distanceUpdate : Int = -1
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Buttons for choosing which mode to use
        val dayMode   = findViewById<Button>(R.id.DayMode)
        ...
        alterMessage = findViewById(R.id.AlterMessage)
        ...
        dayMode.setOnClickListener {
            ModeControl.userModeSelect = 2
        }

        nightMode.setOnClickListener {
            ModeControl.userModeSelect = 1
        }
    ...
    private val bluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onCharacteristicChanged(
            ...
            // When the circuit playground send over the thermo value via BLE
            // If the values are 101 and 102, use these value for high temperature
            Log.i(TAG, characteristic?.value?.get(1)?.toInt().toString())
            handler.post {
                binding.textViewStatus.text =
                    "Obstacle Distance" + characteristic?.value?.get(1)?.toUByte().toString()
            }

            distanceUpdate = characteristic?.value?.get(1)?.toInt()!!

            if ( ModeControl.userModeSelect == 1) {
                if ((distanceUpdate > 50)) {
                    alterMessage.setBackgroundColor(Color.GREEN)
                    alterMessage.text = "Night Mode: Safe\n "
                } else {
                    alterMessage.setBackgroundColor(Color.RED)
                    alterMessage.text = "Night Mode: Caution\n "
                }
            }

            if ( ModeControl.userModeSelect == 2) {
                if ((distanceUpdate > 100)) {
                    alterMessage.setBackgroundColor(Color.GREEN)
                    alterMessage.text = "Day Mode: Safe\n "
                } else {
                    alterMessage.setBackgroundColor(Color.RED)
                    alterMessage.text = "Day Mode: Caution\n "
                }
            }
